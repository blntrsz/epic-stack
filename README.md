...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create epic-stack-3617
  fly apps create epic-stack-3617-staging
  ```
...
- Add a `SESSION_SECRET`, `ENCRYPTION_SECRET`, and `INTERNAL_COMMAND_TOKEN` to
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) ENCRYPTION_SECRET=$(openssl rand -hex 32) INTERNAL_COMMAND_TOKEN=$(openssl rand -hex 32) --app epic-stack-3617
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) ENCRYPTION_SECRET=$(openssl rand -hex 32) INTERNAL_COMMAND_TOKEN=$(openssl rand -hex 32) --app epic-stack-3617-staging
  ```
...
- **Create an account on Mailgun.** (Can be deferred to later)
  ...
  ```sh
  fly secrets set MAILGUN_DOMAIN="mg.example.com" MAILGUN_SENDING_KEY="some-api-token-with-dashes" --app epic-stack-3617
  fly secrets set MAILGUN_DOMAIN="mg.example.com" MAILGUN_SENDING_KEY="some-api-token-with-dashes" --app epic-stack-3617-staging
  ```
...
- Create a persistent volume for the sqlite database for both your staging and
  ...
  ```sh
  fly volumes create data --region sjc --size 1 --app epic-stack-3617
  fly volumes create data --region sjc --size 1 --app epic-stack-3617-staging
  ```
